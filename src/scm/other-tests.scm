...
(define-tests mutation-tests
 ...
 (define-tests io-tests
   ...
   ((begin
      (define out (open-output-file "foo"))
      (write 'hello out)
      (write 123 out)
      (define in (open-input-file "foo"))
      (read in)
      (read in)) => 123)
   ((eof-object? (peek-char (open-input-file "foo"))) => #t)
   ((begin
      (write 'hello (open-output-file "foo"))
      (define in (open-input-file "foo"))
      (and (eqv? (read-char in) #\h)
	   (eqv? (read-char in) #\e)
	   (eqv? (read-char in) #\l)
	   (eqv? (read-char in) #\l)
	   (eqv? (read-char in) #\o)
	   (eof-object? (read-char in))
	   (eof-object? (read-char in)))) => #t)
   ((begin
      (write 123 (open-output-file "foo"))
      (define in (open-input-file "foo"))
      (and (eqv? (peek-char in) #\1)
	   (eqv? (read-char in) #\1)
	   (eqv? (peek-char in) #\2)
	   (eqv? (read-char in) #\2)
	   (eqv? (peek-char in) #\3)
