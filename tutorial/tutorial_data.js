...
var tutorial = (function() {
    ...
    var tutorialObj = new Tutorial()
        ...
        function(input) {
            if (GayLisp.eval('(string? ' + input + ')') === '#t') {
                ...
                tutorialObj.setErrorMessage("Sorry " + name + ", that's not quite what I had in mind."
                ).addStep([
    ...
    return function(string, terminal) {
        if (string === '(tutorial)') {
            terminal.reset()
                .println(tutorialObj.getIntroMessage())
                .popInterpreter();
            terminal.pushInterpreter(function(string, terminal) {
                return tutorialObj.eval(string, terminal);
            });
            return ' ';
        } else return null;
    }
