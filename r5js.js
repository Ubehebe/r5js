...
scan['token'] = function(text, offset) {
    ...
    /* The dot has to be handled differently because it can also be a decimal
     point. We have to peek ahead with requireTokenEnd() to
     distinguish the two. */
    if (unigraph === '.') {
...
scan['decimal'] = function(text, offset) {
    ...
    if (leadingDot) {
        ...
        if (secondDigitBlock.success) {
            var afterSecondHashes = consumeWhile(text, secondDigitBlock.offset, scanHelper.isHash);
        ...
        // <decimal 10> -> <digit 10>+ #+ . #* <suffix>
            ...
            var afterSecondHashes = consumeWhile(text, afterHashes + 1, scanHelper.isHash);
            suffix = scan['suffix'](text, afterSecondHashes);
            return suffix.success
                ? scanOk('decimal', firstDigitBlock.value + '.' + suffix.value, suffix.offset)
                : suffix;
...
function consumeIntertokenSpace(text, offset) {
    ...
    var inComment = false;
    while (true) {
        console.log('consumeIntertokenSpace offset ' + offset);
...
validTokens['number'] = (function() {
    ...
    var nums = ['0', '1@2', '1.23+4/5i', '-5.556-10/9i', '-50-i', '0+i', ''];
    ...
    var decimals = ["8762", "4987566###", ".765", ".549867#", "0.", "37.###", "565.54", "3765.4499##", "4##.", "56#.", "587##.#"];
