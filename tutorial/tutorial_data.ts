...
function evalTrue(input: string) {
  return true; // TODO re-enable return GayLisp.Eval(input) === '#t';
}

function Eval(input: string): any {
  return input; // TODO actually call the interpreter
}

const theSecretOfLisp = " _    ___ ___ _____ ___ \n| |  |_ _/ __|_   _/ __|\n| |__ | |\\__ \\ | | \\__ \\\n|____|___|___/ |_| |___/\n                        \n   _   ___ ___ \n  /_\\ | _ \\ __|\n / _ \\|   / _| \n/_/ \\_\\_|_\\___|\n               \n ___ ___  ___   ___ ___    _   __  __ ___ \n| _ \\ _ \\/ _ \\ / __| _ \\  /_\\ |  \\/  / __|\n|  _/   / (_) | (_ |   / / _ \\| |\\/| \\__ \\\n|_| |_|_\\\\___/ \\___|_|_\\/_/ \\_\\_|  |_|___/\n                                          \n";
const theSecretOfLispFallback = "Lists are programs.";

export const tut = new Tutorial().setRandomCongrats([
  'Great.',
  'OK.',
  'Good.',
  'Good job.',
...
tut.addStep(new Step([
    ...
    (input) => {
      ...
      if (ans) {
        tut.setLocalVar('name', Eval(input));
      }
      return ans;
    },
    [tut.withLocalVar(
    ...
    (input) => evalTrue(`(equal? ${input} '(-10 3 "Cornwall"))`))
).addStep(new Step([
    ...
    (input) => evalTrue(`(null? ${input})`))
      ...
      tut.withLocalVar('name', (name) => `${name}, are you ready to learn the secret of Scheme?`),
      '(Type "yes", with the quotes.)'],
    (input) => Eval('(string=? "yes" ' + input + ')') === '#t',
    ["The secret of Scheme is:", theSecretOfLisp
    ...
    (input) => evalTrue(`(and (= -98 ${input}) (number? (quote ${input})))`),
    ...
    (input) => evalTrue(input + ' (equal? bad-guys (quote ("Albany" "Cornwall")))'),
    ...
    (input) => evalTrue(`(define x ${input}) (and (list? x) (eq? list (car x)) (null? (cdr x)))`),
    [tut.withLocalVar(
        'name',
        (name) => `Good, ${name}. The first list command says to make a list, and the second list command is what actually gets put in that list.`)])
        .disableRandomCongrat()
).addStep(new Step([
    ...
    (input) => {
    ...
    }, [tut.withLocalVar('list-error', (listError) => listError),
      ...
      tut.withLocalVar(
          'name',
          (name) => `Think you understand, ${name}? (There's no hurry. Since I'm living on your computer now, we can spend a lot of time together :))`)],
    (input) => evalTrue(`(string=? "yes" ${input})`)
    ).disableRandomCongrat()
    ...
    (input) => evalTrue(`(and (null? ${input}) (not (eq? 'list (car (quote ${input})))))`))
).addStep(new Step([
    ...
    (input) => evalTrue(`(define x ${input}) (equal? x '(() '() (list) '(list)))`),
    [tut.withLocalVar('name', (name) => `YES!! You did it, ${name}!`)]
    ).disableRandomCongrat()
).addStep(new Step([
    ...
    (input) => {
      if (input === '"go on"') {
        tut.setGoodbye([
          /*tut.withLocalVar('name', (name) => name + */", it's time for you to leave me behind and explore Lisp on your own.",
